...
for img_name in tqdm(np.sort(os.listdir(opt.input_folder))):
    ...
    # -------------------------- 2. 下采样V分量（降低计算量） --------------------------
    img_rgb_lr = interpolate_image(img_rgb, opt.down_size, opt.down_size)
    H_lr, W_lr = img_rgb_lr.shape[2], img_rgb_lr.shape[3]
    ...
    for epoch in range(opt.epochs):
        ...
        # 前向传播：预测光照残差
        illu_res_lr = model(img_rgb_lr)  # (1,1,H_lr,W_lr) → 光照残差
        illu_lr = illu_res_lr + img_rgb_lr  # 预测光照 = V分量 + 残差
        img_rgb_fixed_lr = img_rgb_lr / (illu_lr + 1e-4)  # 增强后的V分量（避免除零）

        # 计算各损失项
        loss_fidelity = F.mse_loss(illu_lr, img_rgb_lr)  # 保真度损失（MSE）
        loss_tv = l_TV(illu_lr)  # 平滑损失（TV）
        loss_exp = l_exp(illu_lr)  # 曝光损失
        loss_sparsity = torch.mean(img_rgb_fixed_lr)  # 稀疏性损失
        loss_mse = nn.MSELoss(img_rgb_fixed_lr, illu_lr)
    ...
    # -------------------------- 6. 上采样恢复原始分辨率 --------------------------
    # 使用引导滤波上采样增强后的V分量
    img_rgb_fixed = filter_up(
        x_lr=img_rgb_lr,    # 低分辨率V分量（引导图）
        y_lr=img_rgb_fixed_lr,  # 低分辨率增强V分量（待上采样图）
        x_hr=img_rgb,       # 原始分辨率V分量（目标分辨率）
        r=1               # 引导滤波半径
    )
